...
# ══════════════════════════════════════════════════════════
#   CUSTOM EMOJI HELPERS  —  FIXED
#   Telegram requires a valid emoji character inside <tg-emoji>.
#   Plain text or space causes Entity_text_invalid error.
#   Fix: nest the fallback custom emoji ID inside every tag.
# ══════════════════════════════════════════════════════════

_DEFAULT_FALLBACK_ID = "5325547803936572038"


def tg(emoji_id: str, fallback: str = None) -> str:
    """Wrap emoji_id in a valid <tg-emoji> tag.
    Inner content must be a real emoji — we use a nested custom emoji
    tag with the fallback ID so Telegram always accepts the entity.
    """
    inner = f'<tg-emoji emoji-id="{_DEFAULT_FALLBACK_ID}">⭐</tg-emoji>'
...
def ce(name: str) -> str:
    """Return a valid <tg-emoji> tag for a logical emoji name.
    Falls back to the default custom emoji tag if name not found.
    """
    entry = CUSTOM_EMOJIS.get(name)
    if not entry or not entry[0]:
        return tg(_DEFAULT_FALLBACK_ID)
    return tg(entry[0])
...
async def button_handler(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    if data == 'main_menu':
        ...
        success, msg = payment.process_wallet_payment(
            user_id, utils.generate_order_id(), product_id, quantity, total_amount
        )
        if success:
...
async def handle_wallet_deposit_amount(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    try:
        ...
        if amount <= 0:
            await update.message.reply_text("❌ Please enter a valid amount greater than 0.", reply_markup=utils.deposit_enter_amount_keyboard())
            return WALLET_DEPOSIT_AMOUNT
        ...
        await update.message.reply_text("❌ Invalid amount. Example: 10.5", reply_markup=utils.deposit_enter_amount_keyboard())
        return WALLET_DEPOSIT_AMOUNT
...
async def handle_wallet_deposit_ref(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    if not amount:
        await update.message.reply_text("❌ Deposit amount missing. Please start deposit again.", reply_markup=utils.wallet_options_keyboard())
        return ConversationHandler.END
    await update.message.reply_text("⏳ Checking Binance Pay history... Please wait.", parse_mode='HTML')
    success, msg = payment.process_wallet_deposit_binance(user_id=user_id, amount=amount, binance_order_id=reference)
    if success:
...
async def handle_withdraw_address(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    await notify_admins(context, f"💸 *New Withdrawal*\n\nUser: `{user_id}`\nAmount: {amount} USDT\nAddress: `{address}`", parse_mode='Markdown')
    return ConversationHandler.END
...
async def handle_contact_admin(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    await notify_admins(context, f"📩 *Message from User*\n\nID: `{user_id}`\nUsername: @{username}\n\n{msg_text}", parse_mode='Markdown')
...
async def handle_admin_add_items(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    try:
        ...
        if products_to_broadcast:
            await update.message.reply_text(f"📢 Stock update notification sent to {sent}/{total} users.", reply_markup=utils.admin_main_keyboard())
    except Exception as e:
...
async def handle_admin_edit_details(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    try:
        ...
        if len(parts) < 3:
            await update.message.reply_text("❌ Invalid format. Use: `product_id field_name new_value`", reply_markup=utils.admin_main_keyboard(), parse_mode='Markdown')
            return ConversationHandler.END
...
async def handle_admin_edit_credentials(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    try:
        parts = update.message.text.split()
        if len(parts) < 2:
            await update.message.reply_text("❌ Invalid format. Use: `item_id key:value ...`", reply_markup=utils.admin_main_keyboard(), parse_mode='Markdown')
            return ConversationHandler.END
        item_id = int(parts[0])
        updates = {}
        for part in parts[1:]:
            if ':' not in part:
                await update.message.reply_text(f"❌ Invalid field format: `{part}`. Use key:value pairs.", reply_markup=utils.admin_main_keyboard(), parse_mode='Markdown')
                return ConversationHandler.END
